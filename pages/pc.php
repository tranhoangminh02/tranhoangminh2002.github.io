...
<div class="container-fluid bg-trasparent mt-1" style="position: relative">
    <div class="h5 p-3 mb-2 bg-danger text-white rounded mt-1">
        <strong>PC VĂN PHÒNG - DOANH NGHIỆP</strong>
    </div>

    <div class="row row-cols-1 row-cols-xs-2 row-cols-sm-2 row-cols-lg-4 g-3">
        <?php
            $result = mysqli_query($conn,"SELECT * FROM sanpham INNER JOIN danhmuc ON sanpham.id_dm = danhmuc.id_dm WHERE ten_dm = 'PC' AND loai_sp = 'Văn phòng - Doanh nghiệp' AND so_luong > 0 LIMIT 8");
    ...
    <div class="row row-cols-1 row-cols-xs-2 row-cols-sm-2 row-cols-lg-4 g-3">
        <?php
            $result = mysqli_query($conn,"SELECT * FROM sanpham INNER JOIN danhmuc ON sanpham.id_dm = danhmuc.id_dm WHERE ten_dm = 'PC' AND loai_sp = 'Gaming' AND so_luong > 0 LIMIT 8");
    ...
    <div class="h5 p-3 mb-2 bg-danger text-white rounded mt-2 mb-5">
        <strong>PC KỸ THUẬT - ĐỒ HOẠ</strong>
    </div>
    <div class="row row-cols-1 row-cols-xs-2 row-cols-sm-2 row-cols-lg-4 g-3">
        <?php
            $result = mysqli_query($conn,"SELECT * FROM sanpham INNER JOIN danhmuc ON sanpham.id_dm = danhmuc.id_dm WHERE ten_dm = 'PC' AND loai_sp = 'Đồ hoạ' AND so_luong > 0 LIMIT 8");
    ...
    <!-- <div class="h5 p-3 mb-2 bg-danger text-white rounded mt-2 mb-5">
        <strong>PC - HÃNG MSI</strong>
    </div>
    <div class="row row-cols-1 row-cols-xs-2 row-cols-sm-2 row-cols-lg-4 g-3">
        <?php
            $result = mysqli_query($conn,"SELECT * FROM sanpham INNER JOIN hangsanxuat ON sanpham.id_hang_sx = hangsanxuat.id_hang_sx  WHERE ten_hang_sx = 'MSI' AND id_dm = 2 AND so_luong > 0 ORDER BY gia ASC LIMIT 8");
